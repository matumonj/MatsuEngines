...
void CameraControl::BossSceneStart()
{
	...
	if (point.size() == 0)
	{
		...
		point.push_back({BossPos.x, BossPos.y + 200.0f, BossPos.z - 140.0f}); //上から
		point.push_back({BossPos.x, BossPos.y + 130.0f, BossPos.z - 100.0f}); //下へ
...
void CameraControl::BossCutScene_Spline()
{
	...
	elapsedTime = elapsedCount / 300.0f;
