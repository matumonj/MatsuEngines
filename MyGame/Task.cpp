...
void Task::Upda()
{
	...
	for (int i = 0; i < TaskNum; i++)
	{
		...
		TaskPos[i] = { 1550.0f,520.f + float(i) * SpritePosInter };
	...
	//後で関数化スべき
		...
		DebugTextSprite2::GetIns()->Print(taskstr[TASK_ONE].str() + amount_Golem, TaskMenuPos.x-20.f, TaskMenuPos.y, taskNumSize);
		const std::string amount_MiniGolem = "/2";
		DebugTextSprite2::GetIns()->Print(taskstr[TASK_TWO].str() + amount_MiniGolem, TaskMenuPos.x-20.f, TaskMenuPos.y+TaskNumInterPos, taskNumSize);
		const std::string amount_Switch = "/4";
		DebugTextSprite2::GetIns()->Print(taskstr[TASK_THREE].str() + amount_Switch, TaskMenuPos.x-20.f, TaskMenuPos.y+TaskNumInterPos*2, taskNumSize);
		const std::string amount_Guardian = "/1";
		DebugTextSprite2::GetIns()->Print(taskstr[TASK_FOUR].str() + amount_Guardian, TaskMenuPos.x-20.f, TaskMenuPos.y+TaskNumInterPos*3, taskNumSize);
...
void Task::NavTaskSequence()
{
	...
	for(auto i=0;i<TargetIcon.size();i++)
	{
		...
		TargetIcon[i]->SetPosition({ l_player->GetPosition().x, l_player->GetPosition().y + 7.0f, l_player->GetPosition().z });
