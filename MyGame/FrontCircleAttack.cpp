...
void FrontCircleAttack::Upda()
{
	...
	for (auto i = 0; i < nail_objses_.size(); i++)
	{
		if (nail_objses_[i].AOETexs != nullptr)
		{
			...
			nail_objses_[i].AOETexs->SetPosition({
				nail_objses_[i].TexPos.x, nail_objses_[i].TexPos.y + 4.f, nail_objses_[i].TexPos.z
			});
...
void FrontCircleAttack::SetDamageArea()
{
	...
	{
		//テクスチャランダム
		if (nail_objses_[i].TexAlpha > 0.f)
		{
			continue;
		}
		nail_objses_[i].TexPos = {
			static_cast<float>(rand() % 100 - 50) * 1.2f, bpos.y, static_cast<float>(rand() % 100 - 50) * 1.2f
		};
	...
	if (nail_objses_[nail_objses_.size() - 1].TexAlpha >= 1.f)
	{
		for (auto i = 0; i < nail_objses_.size(); i++)
		{
			...
			nail_objses_[i].PireEffect->CreateParticle(true, {
				                                           nail_objses_[i].ObjPos.x, 10.f, nail_objses_[i].ObjPos.z
			                                           });
		}
		_phase = PHASE_TWO;
	}
