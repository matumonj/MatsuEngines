...
void FbxLoader::ParseMeshVertices(f_Model* f_Model, FbxMesh* fbxMesh)
{
	...
	f_Model->vertices.resize(controlPointsCount, vert);
	...
	if (materialCount > 0)
	{
		...
		if (material)
		{
			if (material->GetClassId().Is(FbxSurfaceLambert::ClassId))
			{
				...
				f_Model->ambient.x = static_cast<float>(ambient.Get()[0]);
				f_Model->ambient.y = static_cast<float>(ambient.Get()[1]);
				f_Model->ambient.z = static_cast<float>(ambient.Get()[2]);

				//拡散反射光係数
				FbxPropertyT<FbxDouble3> diffuse = lambert->Diffuse;
				f_Model->diffuse.x = static_cast<float>(diffuse.Get()[0]);
				f_Model->diffuse.y = static_cast<float>(diffuse.Get()[1]);
				f_Model->diffuse.z = static_cast<float>(diffuse.Get()[2]);
...
void FbxLoader::ParseSkin(f_Model* f_Model, FbxMesh* fbxMesh)
{
	...
	for (int i = 0; i < clusterCount; i++)
	{
		...
		//vector:それを全頂点分
		std::vector<std::list<WeightSet>> weightLists(f_Model->vertices.size());
