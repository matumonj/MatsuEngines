...
void HUD::SkillButtonInitialize()
{
	...
	coolDownSprite[0] = Sprite::Create(28, {CenterPosition.x + 100.f + 120.f, CenterPosition.y + 120.0f},
	                                   {1.f, 1.f, 1.f, 1.f}, {0.0f, 1.0f});
	coolDownSprite[1] = Sprite::Create(28, {CenterPosition.x + 120.f, CenterPosition.y + 100.f + 120.0f},
	                                   {0.f, 0.f, 0.f, 1.f}, {0.0f, 1.0f});
	coolDownSprite[2] = Sprite::Create(28, {CenterPosition.x - 100.f + 120.f, CenterPosition.y + 120.0f},
	                                   {0.f, 0.f, 0.f, 1.f}, {0.0f, 1.0f});
	coolDownSprite[3] = Sprite::Create(28, {CenterPosition.x + 120.f, CenterPosition.y - 100.f + 120.0f},
	                                   {0.f, 0.f, 0.f, 1.f}, {0.0f, 1.0f});
	for (int i = 0; i < 4; i++)
	{
	...
	for (int i = 0; i < enemy.size(); i++)
	{
		...
		if (enemy[i] != nullptr && EnemyHP_Border_Multi.size() > 0)
		{
			...
			else
			{
				...
				multi_Hpt[i] = 0;
				EnemyHP_Inner_Multi[i]->SetScale({
					Percent::GetParcent(enemy[i]->GetMaxHP(), enemy[i]->GetHP()) / 20.00f, 1.5, 1
				});
			}
