...
void TargetMarker::Update_Tutorial(DebugCamera* camera)
{
	...
	TargetMakerTexture->SetRotation({
		90, EnemyControl::GetInstance()->GetEnemy(EnemyControl::TUTORIAL)[0]->GetRotation().y + rotY, 0
	});
	//}
...
void TargetMarker::Update_PlayScene(DebugCamera* camera)
{
	...
	if (tindex != -1)
	{
		...
		TargetMakerTexture->SetRotation({
			90, EnemyControl::GetInstance()->GetEnemy(EnemyControl::PLAYSCENE)[tindex]->GetRotation().y + rotY, 0
		});
	}
	TargetMakerTexture->SetScale({3, 3, 3});
	TargetMakerTexture->SetPosition(MarkerPosition);
